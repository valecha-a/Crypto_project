...
pub async fn fetch_blocks_from_api() -> Result<Vec<Block>, Box<dyn Error + Send + Sync>> {
    ...
    let response = client
        ...
        .header("X-API-KEY", "BQYJaKNzIyY9QJoTpGQgnL6AfcCmm3Ye")
        .json(&request_body)
        .send()
        .await?;
...
/* --------working but not with off_chain---------
...
pub async fn insert_blocks_into_database(client: &Client, blocks: Vec<Block>) -> Result<(), Box<dyn Error + Send + Sync>> {
    // Prepare the INSERT statement
    let stmt = client
        .prepare(
            "INSERT INTO blocks (
...
pub async fn get_blocks_from_database() -> Result<Vec<Block>, tokio_postgres::Error> {
    ...
    let rows = client
        .query(
            "SELECT height, block_hash, block_size, block_weight, block_version, block_stripped_size, difficulty, transaction_count FROM blocks ORDER BY height DESC LIMIT 100",
            &[],
        )
        .await?;
